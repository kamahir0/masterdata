...
const base={path:'type.yaml',name:'Rarity',category:'Enum',underlying:'ulong',conversions:null,members:[{name:'Rare',value:'18446744073709551615'},{name:'Common',value:'0'}],fields:[],fieldTypes:['int','ulong','string'],initializerShapes:{int:{kind:'primitive',primitive:'int'},ulong:{kind:'primitive',primitive:'ulong'},string:{kind:'primitive',primitive:'string'}}};
let snapshot:any;let plan:any;let outcome:any;
const onResult=vi.fn(async()=>{});const beginApply=vi.fn(()=>true);const endApply=vi.fn();
beforeEach(()=>{snapshot=structuredClone(base);plan={token:'1',target:'Rarity',operation:'RenameEnumMember',selector:'Rare',destructive:false,affectedOccurrenceCount:2,files:[{path:'data.yaml',before:'Rare # before',after:'Epic # before'}],diagnostics:[],compatibility:{report:{summary:{changeCount:2,generatedApi:[{classification:'breaking',count:1}],sourceMigration:[{classification:'supported_operation',count:1}],artifactBinary:[{classification:'rebuild_required',count:1}],externalContract:[{classification:'not_assessed',count:1}]}},diagnostic:null}};outcome={state:'success',files:['data.yaml']};invoke.mockReset();onResult.mockClear();beginApply.mockClear();endApply.mockClear();invoke.mockImplementation(async command=>{
...
test('custom initializer is typed and modifier changes invalidate it',async()=>{
  snapshot.category='Custom Type';snapshot.members=[];snapshot.fields=[];
  ...
  fireEvent.mouseDown(screen.getByRole('combobox',{name:'Field modifier'}));
  fireEvent.click((await screen.findAllByText('nullable',{selector:'.ant-select-item-option-content'})).at(-1)!);
  expect((screen.getByRole('checkbox',{name:'Explicit constant initializer'}) as HTMLInputElement).checked).toBe(false);
});

test('Type Migration Plan renders shared released compatibility impact',async()=>{

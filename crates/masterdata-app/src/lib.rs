...
fn is_path_ancestor_or_equal(candidate_path: &Path, target_path: &Path) -> bool {
    let case_sensitivity =
        path_case_sensitivity_for(candidate_path).unwrap_or(PathCaseSensitivity::Sensitive);
    is_path_ancestor_or_equal_with_case(candidate_path, target_path, case_sensitivity)
}

fn is_path_ancestor_or_equal_with_case(
    candidate_path: &Path,
    target_path: &Path,
    ...
    if normalized_path_is_ancestor_or_equal(&candidate, &target, case_sensitivity) {
        return true;
    }
...
enum PathCaseSensitivity {
    Sensitive,
    Insensitive,
}

fn path_case_sensitivity_for(path: &Path) -> Result<PathCaseSensitivity> {
    // WHY: Collision checks include paths that do not exist yet, so lexical
    // spelling alone cannot reveal whether two future artifacts share a
    // directory entry on the target filesystem.
    // IF REMOVED: a case-variant path could bypass ownership and artifact
    // collision checks on a case-insensitive volume.
    // EVIDENCE: docs/specs/build-pipeline.md; Regression: filesystem_case_sensitivity_probe_matches_case_variant_lookup.
    let directory = existing_directory_for_case_probe(path).ok_or_else(|| {
        generated_publish_error(
            path,
            "could not find an existing directory for filesystem case-sensitivity probe",
        )
    })?;
    let probe = TempFileBuilder::new()
        .prefix(".masterdata-case-probe-")
        .tempfile_in(&directory)
        .map_err(|error| {
            generated_publish_error(
                &directory,
                format!("could not probe filesystem case sensitivity: {error}"),
            )
        })?;
    let probe_name = probe.path().file_name().ok_or_else(|| {
        generated_publish_error(
            &directory,
            "could not determine filesystem case-sensitivity probe name",
        )
    })?;
    let variant_name = ascii_case_variant(probe_name).ok_or_else(|| {
        generated_publish_error(
            &directory,
            "could not create filesystem case-sensitivity probe variant",
        )
    })?;
    let variant_path = directory.join(variant_name);
    let variant_exists = match fs::symlink_metadata(&variant_path) {
        Ok(_) => true,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => false,
        Err(error) => {
            return Err(generated_publish_error(
                &variant_path,
                format!("could not inspect filesystem case-sensitivity probe: {error}"),
            ));
        }
    };
    Ok(if variant_exists {
...
fn existing_directory_for_case_probe(path: &Path) -> Option<PathBuf> {
...
fn ascii_case_variant(name: &OsStr) -> Option<OsString> {
...
fn normalized_path_is_ancestor_or_equal(
    ...
    candidate
        ...
        .all(|(candidate, target)| path_component_equal(candidate, target, case_sensitivity))
}
...
fn optional_path_component_equal(
    left: Option<&OsStr>,
    right: Option<&OsStr>,
    case_sensitivity: PathCaseSensitivity,
) -> bool {
    match (left, right) {
        (Some(left), Some(right)) => path_component_equal(left, right, case_sensitivity),
        (None, None) => true,
        _ => false,
    }
}

fn path_component_equal(
    left: &OsStr,
    right: &OsStr,
    case_sensitivity: PathCaseSensitivity,
) -> bool {
    if case_sensitivity == PathCaseSensitivity::Sensitive {
        return left == right;
    }
    ...
    #[cfg(windows)]
    {
        path_component_equal_ignore_case(left, right)
    }
}

#[cfg(windows)]
fn path_component_equal_ignore_case(left: &OsStr, right: &OsStr) -> bool {
...
fn relative_path_is_ancestor_or_equal(
    candidate: &Path,
    target: &Path,
    case_sensitivity: PathCaseSensitivity,
) -> bool {
...
fn relative_paths_equal(left: &Path, right: &Path, case_sensitivity: PathCaseSensitivity) -> bool {
    relative_path_is_ancestor_or_equal(left, right, case_sensitivity)
        && relative_path_is_ancestor_or_equal(right, left, case_sensitivity)
}

fn relative_paths_overlap(
    left: &Path,
    right: &Path,
    case_sensitivity: PathCaseSensitivity,
) -> bool {
...
fn validate_generated_plan_paths(
    ...
    for file in &plan.files {
        ...
        if relative_paths_overlap(&file.relative_path, manifest_path, case_sensitivity) {
        ...
        if let Some(existing) = paths
            .iter()
            .find(|existing| relative_paths_equal(existing, &file.relative_path, case_sensitivity))
        {
            return Err(artifact_path_collision(
                &file.relative_path,
                format!(
    ...
    for (index, path) in paths.iter().enumerate() {
        for other in paths.iter().skip(index + 1) {
            if relative_paths_overlap(path, other, case_sensitivity) {
                return Err(artifact_path_collision(
                    path,
                    format!(
...
fn validate_binary_output_layout(
    ...
    if is_path_ancestor_or_equal_with_case(binary_output, generated_output, case_sensitivity) {
        return Err(artifact_path_collision(
            binary_output,
            format!(
    ...
    if is_path_ancestor_or_equal_with_case(binary_output, &manifest_path, case_sensitivity)
        || is_path_ancestor_or_equal_with_case(&manifest_path, binary_output, case_sensitivity)
    {
        return Err(artifact_path_collision(
            binary_output,
            format!(
    ...
    for generated_path in generated_paths {
        ...
        if is_path_ancestor_or_equal_with_case(
            binary_output,
            &final_generated_path,
            case_sensitivity,
        ) || is_path_ancestor_or_equal_with_case(
            &final_generated_path,
            binary_output,
            case_sensitivity,
        ) {
...
fn validate_unmanaged_generated_collisions(
    ...
    if fs::symlink_metadata(output_dir).is_ok() {
        collect_existing_output_entries(output_dir, Path::new(""), &mut entries, case_sensitivity)?;
    }
    ...
    for generated_path in generated_paths {
        for entry in &entries {
            let managed_entry = managed_path_matches(&entry.relative, managed, case_sensitivity);
            if relative_paths_equal(generated_path, &entry.relative, case_sensitivity) {
            ...
            if relative_path_is_ancestor_or_equal(&entry.relative, generated_path, case_sensitivity)
            ...
            if relative_path_is_ancestor_or_equal(generated_path, &entry.relative, case_sensitivity)
                && !relative_paths_equal(generated_path, &entry.relative, case_sensitivity)
            {
                return Err(generated_ownership_collision(
                    output_dir,
                    generated_path,
...
fn validate_unmanaged_binary_collisions(
    output_dir: &Path,
    binary_output: &Path,
    managed: &BTreeSet<String>,
    case_sensitivity: PathCaseSensitivity,
) -> Result<()> {
    let binary_is_nested =
        is_path_ancestor_or_equal_with_case(output_dir, binary_output, case_sensitivity)
            && !is_path_ancestor_or_equal_with_case(binary_output, output_dir, case_sensitivity);
    if !binary_is_nested || fs::symlink_metadata(output_dir).is_err() {
        return Ok(());
    }

    // WHY: A binary nested under generated output must respect the same
    // unmanaged ownership boundary as generated files. The normal binary
    // path outside generated output is intentionally not treated as an
    // unmanaged entry because replacing a previous published binary is part
    // of the existing publication flow.
    // IF REMOVED: a nested binary destination could overwrite a user-owned
    // file or replace a user-owned directory subtree after validation.
    // EVIDENCE: docs/specs/build-pipeline.md; Regression: binary_output_rejects_unmanaged_existing_file.
    let mut entries = Vec::new();
    collect_existing_output_entries(output_dir, Path::new(""), &mut entries, case_sensitivity)?;
    for entry in entries {
        let existing_path = output_dir.join(&entry.relative);
        let overlaps =
            is_path_ancestor_or_equal_with_case(binary_output, &existing_path, case_sensitivity)
                || is_path_ancestor_or_equal_with_case(
                    &existing_path,
                    binary_output,
                    case_sensitivity,
        ...
        let managed_file = entry.kind == ExistingOutputEntryKind::File
            && managed_path_matches(&entry.relative, managed, case_sensitivity);

        if managed_file && same_path {
            continue;
        }

        // A binary file cannot replace an existing directory, nor can it be
        // placed at an existing file's path unless that exact file is already
        // publisher-managed.
        if entry.kind == ExistingOutputEntryKind::Directory
            && !is_path_ancestor_or_equal_with_case(binary_output, &existing_path, case_sensitivity)
        {
            continue;
        }
        ...
        return Err(artifact_path_collision(
            binary_output,
            format!(
                "binary output `{}` overlaps unmanaged existing generated output entry `{}`; user-owned content will not be overwritten",
                binary_output.display(),
                existing_path.display(),
            ),
        ));
...
fn collect_existing_output_entries(
    ...
    for entry in fs::read_dir(directory).map_err(|error| {
        ...
        if relative_directory.as_os_str().is_empty()
            && relative_paths_equal(
                &relative,
                Path::new(GENERATED_MANIFEST_FILENAME),
                case_sensitivity,
            )
        {
            continue;
        ...
        if file_type.is_dir() {
            ...
            collect_existing_output_entries(&source, &relative, entries, case_sensitivity)?;
...
fn managed_path_matches(
    ...
    managed
        .iter()
        .any(|managed| relative_paths_equal(path, Path::new(managed), case_sensitivity))
}
...
fn validate_artifact_layout(
    ...
    let case_sensitivity = path_case_sensitivity_for(generated_output)?;
    let generated_paths = validate_generated_plan_paths(plan, case_sensitivity)?;
    validate_binary_output_layout(
        generated_output,
        binary_output,
        &generated_paths,
        case_sensitivity,
    )?;
    let managed = read_managed_generated_paths(generated_output, case_sensitivity)?;
    validate_unmanaged_generated_collisions(
        generated_output,
        &managed,
        &generated_paths,
        case_sensitivity,
    )?;
    validate_unmanaged_binary_collisions(
        generated_output,
        binary_output,
        &managed,
        case_sensitivity,
...
fn read_managed_generated_paths(
    output_dir: &Path,
    case_sensitivity: PathCaseSensitivity,
    ...
    for path in manifest.files {
        ...
        if managed_path_matches(Path::new(&path), &managed, case_sensitivity) {
            return Err(generated_publish_error(
                &manifest_path,
...
fn materialize_generated_output(
    ...
    let case_sensitivity = path_case_sensitivity_for(final_output)?;
    let generated_paths = validate_generated_plan_paths(plan, case_sensitivity)?;
    let managed = read_managed_generated_paths(final_output, case_sensitivity)?;
    validate_unmanaged_generated_collisions(
        final_output,
        &managed,
        &generated_paths,
    ...
    if final_output.exists() {
        copy_unmanaged_output(final_output, staged_output, &managed, case_sensitivity)?;
    }
...
fn copy_unmanaged_entry(
    ...
    if managed_path_matches(relative, managed, case_sensitivity) {
        return Ok(());
    }
    let file_type = fs::symlink_metadata(source)
        .map_err(|error| {
...
mod tests {
    ...
    use super::{
        ...
        GENERATED_MANIFEST_VERSION, GeneratedManifest, PathCaseSensitivity, atomic_replace,
        materialize_generated_output, path_case_sensitivity_for, publish_artifacts,
        publish_generated_directory, validate_artifact_layout,
    };
    ...
    #[test]
    fn binary_output_rejects_unmanaged_existing_file() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let binary = generated.join("UserNotes.txt");
        fs::create_dir_all(&generated).expect("generated");
        fs::write(generated.join("UserNotes.txt"), b"KEEP-ME").expect("user file");
        let plan = generation_plan(&[("Item.g.cs", "NEW-GENERATED")]);

        let error = validate_artifact_layout(&plan, &generated, &binary)
            .expect_err("binary/user file collision");
    ...
    #[test]
    fn binary_unmanaged_collision_stops_before_dotnet() {
        let directory = test_project("binary_output = \".masterdata/generated/UserNotes.txt\"\n");
        let generated = directory.path().join(".masterdata/generated");
        fs::create_dir_all(&generated).expect("generated");
        fs::write(generated.join("UserNotes.txt"), b"KEEP-ME").expect("user file");
    ...
    #[test]
    fn binary_output_rejects_unmanaged_file_ancestor() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let binary = generated.join("artifacts/masterdata.bytes");
        fs::create_dir_all(&generated).expect("generated");
        fs::write(generated.join("artifacts"), b"KEEP-ME").expect("blocking file");
        let plan = generation_plan(&[("Item.g.cs", "NEW-GENERATED")]);

        let error = validate_artifact_layout(&plan, &generated, &binary)
            .expect_err("binary ancestor file collision");

        assert_eq!(error.diagnostic().code, "E-BUILD-ARTIFACT-PATH-COLLISION");
        assert_eq!(
            fs::read(generated.join("artifacts")).expect("blocking file"),
            b"KEEP-ME"
        );
    }

    #[test]
    fn binary_output_rejects_replacing_unmanaged_directory() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let binary = generated.join("artifacts");
        fs::create_dir_all(generated.join("artifacts")).expect("user directory");
        fs::write(generated.join("artifacts/notes.txt"), b"KEEP-ME").expect("user file");
        let plan = generation_plan(&[("Item.g.cs", "NEW-GENERATED")]);

        let error = validate_artifact_layout(&plan, &generated, &binary)
            .expect_err("binary directory replacement");

        assert_eq!(error.diagnostic().code, "E-BUILD-ARTIFACT-PATH-COLLISION");
        assert_eq!(
            fs::read(generated.join("artifacts/notes.txt")).expect("user file"),
            b"KEEP-ME"
        );
    }

    #[test]
    fn unrelated_unmanaged_file_allows_nested_binary_output() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let binary = generated.join("artifacts/masterdata.bytes");
        fs::create_dir_all(&generated).expect("generated");
        ...
        validate_artifact_layout(&plan, &generated, &binary)
            .expect("unrelated nested binary output");
        assert_eq!(
            fs::read(generated.join("UserNotes.txt")).expect("user file"),
            b"KEEP-ME"
        );
    ...
    #[test]
    fn filesystem_case_sensitivity_probe_matches_case_variant_lookup() {
        let directory = tempdir().expect("temporary directory");
        let sensitivity =
            path_case_sensitivity_for(directory.path()).expect("filesystem case sensitivity");
        let lower = directory.path().join("caseprobe");
        let upper = directory.path().join("Caseprobe");
        fs::write(&lower, b"probe").expect("case probe file");

        assert_eq!(
            fs::symlink_metadata(&upper).is_ok(),
            sensitivity == PathCaseSensitivity::Insensitive
        );
    }

    #[test]
    fn case_variant_unmanaged_generated_path_follows_filesystem_behavior() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let staged = directory.path().join("staged-generated");
        fs::create_dir_all(&generated).expect("generated");
        ...
        let plan = generation_plan(&[("Item.g.cs", "NEW-GENERATED")]);
        let sensitivity =
            path_case_sensitivity_for(&generated).expect("filesystem case sensitivity");

        let result =
            materialize_generated_output(&CSharpGenerator::default(), &plan, &generated, &staged);
        if sensitivity == PathCaseSensitivity::Insensitive {
            let error = result.expect_err("case-variant unmanaged collision");
            assert_eq!(
                error.diagnostic().code,
                "E-BUILD-GENERATED-OWNERSHIP-COLLISION"
            );
            ...
            result.expect("distinct case-sensitive generated path");
        }
    }

    #[test]
    fn case_variant_binary_generated_path_follows_filesystem_behavior() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        ...
        let result = validate_artifact_layout(&plan, &generated, &binary);
        if sensitivity == PathCaseSensitivity::Insensitive {
            let error = result.expect_err("case-variant binary/generated collision");
            assert_eq!(error.diagnostic().code, "E-BUILD-ARTIFACT-PATH-COLLISION");
        } else {
            result.expect("distinct case-sensitive binary path");
        }
    }

    #[test]
    fn case_variant_manifest_path_remains_reserved() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let binary = generated.join(".MASTERDATA-GENERATED-FILES.JSON");
        fs::create_dir_all(&generated).expect("generated");
        let manifest = GeneratedManifest {
            version: GENERATED_MANIFEST_VERSION,
            files: vec!["Item.g.cs".to_owned()],
        };
        fs::write(
            generated.join(".Masterdata-generated-files.json"),
        ...
        let result = validate_artifact_layout(&plan, &generated, &binary);
        if sensitivity == PathCaseSensitivity::Insensitive {
            let error = result.expect_err("case-variant manifest collision");
            assert_eq!(error.diagnostic().code, "E-BUILD-ARTIFACT-PATH-COLLISION");
        } else {
            result.expect("distinct case-sensitive manifest path");
        }
    }

    #[test]
    fn case_variant_binary_unmanaged_path_follows_filesystem_behavior() {
        let directory = tempdir().expect("temporary directory");
        let generated = directory.path().join("Generated");
        let binary = generated.join("UserData.bin");
        fs::create_dir_all(&generated).expect("generated");
        fs::write(generated.join("userdata.bin"), b"USER-OWNED").expect("user file");
        let plan = generation_plan(&[("Item.g.cs", "NEW-GENERATED")]);
        let sensitivity =
            path_case_sensitivity_for(&generated).expect("filesystem case sensitivity");
        ...
        let result = validate_artifact_layout(&plan, &generated, &binary);
        if sensitivity == PathCaseSensitivity::Insensitive {
            let error = result.expect_err("case-variant binary/unmanaged collision");
            assert_eq!(error.diagnostic().code, "E-BUILD-ARTIFACT-PATH-COLLISION");
            assert_eq!(
                fs::read(generated.join("userdata.bin")).expect("user file"),
                b"USER-OWNED"
            );
        } else {
            result.expect("distinct case-sensitive binary path");
    ...
    #[test]
    fn case_variant_generated_plan_paths_follow_filesystem_behavior() {
        let directory = tempdir().expect("temporary directory");
        let binary = directory.path().join("masterdata.bytes");
        let plan = generation_plan(&[("Item.g.cs", "FIRST"), ("item.g.cs", "SECOND")]);
        let sensitivity =
            path_case_sensitivity_for(directory.path()).expect("filesystem case sensitivity");

        let result = validate_artifact_layout(&plan, directory.path(), &binary);
        if sensitivity == PathCaseSensitivity::Insensitive {
            let error = result.expect_err("case-variant generated plan collision");
            assert_eq!(error.diagnostic().code, "E-BUILD-ARTIFACT-PATH-COLLISION");
        } else {
            result.expect("distinct case-sensitive generated paths");
        }
    }

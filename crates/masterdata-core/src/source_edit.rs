...
/// Hash exact UTF-8 source bytes. This is an editor snapshot identity, not a
...
/// Prepare an existing-record value edit against an already loaded snapshot.
///
/// The function is deliberately filesystem-independent. Host adapters supply a
/// snapshot and commit the returned candidate through their own I/O boundary.
pub fn dry_run_source_edit(
    ...
    for edit in edits {
        let target = (edit.record_index, edit.field.clone());
        ...
        let span = locate_record_member_value(
            &loaded.source,
            data,
            edit.record_index,
            &edit.field,
    ...
    let reparsed = parse_yaml_document(path.to_path_buf(), &candidate_source).map_err(|error| {
        with_requirement(error, "SOURCE-EDIT-005")
    })?;
    ...
    let index = transformed_documents
        ...
        .expect("target file was resolved above");
...
fn editable_fields(schema: &SchemaDocument) -> BTreeMap<&str, PrimitiveType> {
    ...
    schema
        ...
        .iter()
        .filter_map(|field| {
            (!field.nullable && !field.array && !keys.contains(field.name.as_str()))
                .then(|| PrimitiveType::parse(&field.type_name).map(|primitive| (field.name.as_str(), primitive)))
                .flatten()
        })
        .collect()
}
...
fn locate_record_member_value(
    ...
    let records_line = find_top_level_key(&lines, "records").ok_or_else(|| source_edit_error(
    ...
    let item_line = *sequence.items.get(record_index).ok_or_else(|| source_edit_error(
        "E-SOURCE-EDIT-RECORD-NOT-FOUND",
    ...
    let mut end = line.start + mapping.value_end;
...
fn literal_span(
    ...
    let mapping = mapping_span(header.text).ok_or_else(|| source_edit_error(
        "E-SOURCE-EDIT-SOURCE-UNCLASSIFIABLE",
    ...
    while body_end_line < record_end
        && literal_content.get(body_end_line).copied().unwrap_or(false)
    {
        body_end_line += 1;
    }
...
fn render_replacement(
    ...
    if let Some(literal) = &span.literal {
        ...
        return serde_json::to_string(input).map_err(|error| source_edit_error(
...
fn json_string(input: &str) -> Result<String> {
    serde_json::to_string(input).map_err(|error| source_edit_error(
        "E-SOURCE-EDIT-SCALAR-RENDER",
        format!("could not render edited string scalar: {error}"),
        None,
        "SOURCE-EDIT-003",
...
fn mapping_span(line: &str) -> Option<MappingSpan> {
    ...
    let raw_value = code[value_start..value_end].to_owned();
    Some(MappingSpan {
        key,
        value_start,
        value_end,
    ...
    while index < bytes.len() {
        match quote {
            ...
            None => match bytes[index] {
                ...
                b':' if bytes.get(index + 1).is_none_or(|next| next.is_ascii_whitespace()) => {
                    return Some(index);
                }
                _ => index += 1,
            },
...
"#;
    ...
    fn source_edit_preserves_unrelated_text_and_exact_ulong() {
        ...
        assert!(dry_run.plan.candidate_source.contains(
            "weight: 18446744073709551615 # inline\r\n    note: 'it''s ready'\r\n"
        ));
    ...
    fn source_edit_allows_domain_invalid_scalar_and_validation_reports_it() {
        ...
        assert!(dry_run.plan.candidate_source.contains("weight: not-a-number"));

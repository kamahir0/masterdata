...
- **Field ID**: 旧仕様が定義していた、container内でfieldを追跡するpersistentなnumeric identity。active/reserved namespace、tombstone、reuse prohibition、rename trackingを含む旧modelはretiredであり、現行contractではない。現行modelは`SCHEMA-KEY-001`へrouteする。
